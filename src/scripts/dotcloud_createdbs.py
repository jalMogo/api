...
def create_dbs():
    ...
    if not databases:
        raise RuntimeError("No databases in %s" % os.environ["DJANGO_SETTINGS_MODULE"])
    for name, db in databases.iteritems():
        ...
        if db_type.endswith("mysql"):
            ...
            db = MySQLdb.connect(user=user, passwd=password, host=host, port=port)
            cur = db.cursor()
        ...
        # see if it is postgresql
            ...
            con = psycopg2.connect(
                host=host, user=user, password=password, port=port, database="postgres"
            )
