...
validate_color = RegexValidator(
    regex=color_re, message="Enter a valid color.", code="invalid"
)
...
class Tag(ClosureModel):
    ...
    color = models.CharField(
        max_length=7, validators=[validate_color], null=True, blank=True
    )
...
class PlaceTag(TimeStampedModel):
    tag = models.ForeignKey(
        Tag, related_name="place_tags", null=False, on_delete=models.CASCADE
    )
    ...
    def clean(self):
        if (
            hasattr(self, "tag")
            and hasattr(self, "place")
            and self.tag.dataset.id != self.place.dataset.id
        ):
            raise ValidationError(
                "The Tag must come from the same DataSet as the Place."
            )
