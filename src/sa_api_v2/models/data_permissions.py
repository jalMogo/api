...
class DataPermission(CloneableModelMixin, CacheClearingModel, models.Model):
    ...
    def parent():
        ...
        def fset(self, value):
            setattr(self, self.parent_attr, value)
    ...
    def siblings():
        def fget(self):
            return self.parent.permissions.all()
    ...
    def get_abilities(self):
        abilities = []
        if self.can_create:
            abilities.append("create")
        if self.can_retrieve:
            abilities.append("retrieve")
        if self.can_update:
            abilities.append("update")
        if self.can_destroy:
            abilities.append("destroy")
        if self.can_access_protected:
            abilities.append("can_access_protected")
    ...
    def abilities(self):
        ...
        things = (
            self.submission_set
            if self.submission_set.strip() not in ("", "*")
            else "anything"
        )

        if abilities:
            if len(abilities) > 1:
                abilities[-1] = "or " + abilities[-1]
...
post_save.connect(
    create_data_permissions, sender=DataSet, dispatch_uid="dataset-create-permissions"
)
...
def any_allow(permissions, do_action, resource, protected=False):
    ...
    for permission in permissions:
        ...
        if (
            permission.submission_set in (resource, "*")
...
def check_data_permission(
    user, client, place_id, do_action, dataset, resource, protected=False
):
    """
    ...
    if (
        dataset
        and dataset.auth_required
        and do_action == "create"
        and (not user or not user.is_authenticated())
    ):
        return False
    ...
    if client is not None:
        if client.dataset == dataset and any_allow(
            client.permissions.all(), do_action, resource, protected
        ):
            return True
    ...
    if user is not None and user.is_authenticated():
        for group in user._groups.all():
            if (
                dataset
                and group.dataset_id == dataset.id
                and any_allow(group.permissions.all(), do_action, resource, protected)
            ):
                return True
