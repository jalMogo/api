...
class ShareaboutsUserManager(UserManager):
    def get_queryset(self):
        return (
            super(ShareaboutsUserManager, self)
            .get_queryset()
            .prefetch_related("_groups")
        )
    ...
    def get_twitter_access_token(self):
        ...
        if token_response.status_code != 200:
            raise Exception(
                "Received a {0} response while retrieving Twitter "
    ...
    def ensure_social_user(
        ...
        try:
            social_auth = UserSocialAuth.objects.get(provider=provider, uid=user_id)
            ...
            social_auth = UserSocialAuth(
                user=user, provider=provider, uid=user_id, extra_data=extra_data or {}
            )
            social_auth.save()
    ...
    def add_twitter_user(self, username, firstname=None, lastname=None):
        ...
        if response.status_code != 200:
            raise Exception(
                "Received a {0} response while trying to get user "
        ...
        return self.ensure_social_user(
            "twitter", user_id, username, social_data, firstname, lastname
        )
    ...
    def add_third_party_user(self, provider, username, firstname, lastname):
        if provider == "twitter":
            ...
            raise NotImplemented(
                "Adding by {0} is not yet implemented".format(provider)
            )
