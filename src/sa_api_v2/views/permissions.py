...
def is_really_logged_in(user, request):
    ...
    return (
        ...
        and not is_apikey_auth(auth)
        and not is_origin_auth(auth)
    )
...
class IsOwnerOrReadOnly(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if (
            ...
            or is_owner(request.user, request)
            or request.user.is_superuser
            or (
                hasattr(request, "client")
                and
                # check if the request's dataset's owner matches the CORS origin:
                hasattr(request.client, "owner")
                and is_owner(request.client.owner, request)
            )
        ):
            return True
        return False
...
class IsAllowedByDataPermissions(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if (
            hasattr(view, "model") and issubclass(view.model, models.DataSet)
        ) or JWT_PARAM in request.GET:
            return True
        ...
        if (
            hasattr(view, "submission_set_name_kwarg")
            and view.submission_set_name_kwarg in view.kwargs
        ):
        ...
        protected = (
            INCLUDE_INVISIBLE_PARAM in request.GET
            or INCLUDE_PRIVATE_FIELDS_PARAM in request.GET
            or INCLUDE_PRIVATE_PLACES_PARAM in request.GET
        )
        ...
        return models.check_data_permission(
            user, client, place_id, do_action, dataset, data_type, protected
        )
