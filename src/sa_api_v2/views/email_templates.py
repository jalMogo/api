...
class EmailTemplateMixin(object):
    def send_email_notification(self, obj, submission_set_name):
        ...
        email_templates = [
            origin.place_email_template
            for origin in obj.dataset.origins.all()
            if cors.models.Origin.match(origin.pattern, request_origin)
            and origin.place_email_template is not None
        ]

        filtered_email_templates = [
            template
            for template in email_templates
            if template.submission_set in [submission_set_name, ""]
            and template.event == "add"
        ]
        ...
        for email_template in filtered_email_templates:
            ...
            if email_field != "" and email_field in self.request.data:
                ...
                logger.debug(
                    "[EMAIL] No primary recipient found. Setting primary recipient to the default."
                )
            ...
            if not recipient_email and not bcc_list:
                errors.append(
                    "[EMAIL] Error: No primary recipient and no BCC recipients provided. Email will not be sent."
                )
            ...
            msg = EmailMultiAlternatives(
                subject, body, from_email, to=[recipient_email], bcc=bcc_list
            )
