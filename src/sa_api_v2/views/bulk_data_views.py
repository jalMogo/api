...
class DataSnapshotRequestListView(DataSnapshotMixin, OwnedResourceMixin, views.APIView):
    ...
    **Authentication**: Basic, session, or key auth *(required)*
    ...
    def get_method_actions(self):
        return {"GET": "retrieve", "PUT": "retrieve", "POST": "retrieve"}

    def get_recent_requests(self, characteristic_params):
        return (
            DataSnapshotRequest.objects.all()
    ...
    def get_most_recent_request(self, characteristic_params):
        try:
            return self.get_recent_requests(characteristic_params).filter(
                status="pending"
            )[0]
        except IndexError:
    ...
    def initiate_data_request(self, characteristic_params):
        ...
        task = store_bulk_data.apply_async(
            args=(datarequest.pk,),
            link=bulk_data_status_update.s(),
            link_error=bulk_data_status_update.s(),
        )
    ...
    def get_characteristic_params(
        self, request, owner_username, dataset_slug, submission_set_name
    ):
        """
        ...
        return {
            ...
            "include_private_fields": str(
                params.get("include_private_fields", False)
            ).lower()
            not in ("f", "false", "off"),
            "include_invisible": str(params.get("include_invisible", False)).lower()
            not in ("f", "false", "off"),
            "include_submissions": str(params.get("include_submissions", False)).lower()
            not in ("f", "false", "off"),
        }

    def get_or_create_datarequest(
        self, request, owner_username, dataset_slug, submission_set_name
    ):
        characteristic_params = self.get_characteristic_params(
            request, owner_username, dataset_slug, submission_set_name
        )
    ...
    def post(self, request, owner_username, dataset_slug, submission_set_name):
        datarequest = self.get_or_create_datarequest(
            request, owner_username, dataset_slug, submission_set_name
        )
    ...
    def get(self, request, owner_username, dataset_slug, submission_set_name):
        ...
        return Response(
            [self.get_data_description(datarequest) for datarequest in datarequests],
            status=200,
        )
    ...
    **Authentication**: Basic, session, or key auth *(required)*
    ...
    def get(
        ...
        try:
            ...
            return Response(
                {"status": "not found", "message": "This data is no longer available"},
                status=404,
            )
        ...
        if format not in ("json", "geojson", "csv"):
            return Response({"message": "Invalid format: %s" % (format,)}, status=400)
