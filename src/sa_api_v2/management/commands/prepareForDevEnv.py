...
from sa_api_v2.models import DataSet, OriginPermission, PlaceEmailTemplate
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            for email_template in PlaceEmailTemplate.objects.all():
                logger.info(
                    "Deleting origin references to place email template: {}".format(
                        email_template
                    )
                )
            ...
            for dataset in datasets:
                ...
                matches = [
                    origin
                    for origin in dataset.origins.all()
                    if origin is not None and "localhost:8000" in origin.pattern
                ]
                if len(matches) > 0:
                    logger.info(
                        "dataset already has localhost:8000 origin: {}".format(dataset)
                    )
                    continue

                # create a new permission:
                ds_origin = Origin.objects.create(
                    pattern="localhost:8000", dataset=dataset
                )
