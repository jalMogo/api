...
def parse_env(dict):
    ...
    for line in content.splitlines():
        m1 = re.match(r"\A([A-Za-z_0-9]+){}=(.*)\Z".format(DEV_KEY_SUFFIX), line)
        if m1:
            ...
            logger.info("parsing key from environment file: {}={}".format(key, val))
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dataset in datasets:
            api_key_value = api_key_values.get(
                "{}{}".format(dataset.display_name.upper(), DEV_KEY_SUFFIX), None
            )
            # handle case when we have a dataset but no dev key:
            if api_key_value is None:
                logger.error(
                    "No matching key found for dataset: {}".format(dataset.display_name)
                )
            ...
            if dataset.keys is None or len(dataset.keys.all()) < 1:
                logger.error(
                    "Skipping dataset because it has no api key: {}\n".format(
            ...
            logger.info(
                "setting key for datset name: {} to value: {}".format(
                    dataset.display_name, api_key_value
                )
            )
