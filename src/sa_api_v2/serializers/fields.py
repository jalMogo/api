    ...
    def to_internal_value(self, data):
        ...
        try:
            ...
            raise ValidationError(
                "Problem converting native data to Geometry: %s" % (exc,)
            )
        ...
        for arg_name in self.url_arg_names:
            ...
            if arg_value is None:
                try:
                    ...
                    raise KeyError(
                        "No arg named %r in %r" % (arg_name, instance_kwargs)
                    )
...
class ShareaboutsRelatedField(
    ShareaboutsFieldMixin, serializers.HyperlinkedRelatedField
):
    """
...
class TagRelatedField(serializers.HyperlinkedRelatedField):
    ...
    def get_url(self, obj, view_name, request, format):
        ...
        return reverse(
            self.view_name, kwargs=url_kwargs, request=request, format=format
        )
...
class ShareaboutsIdentityField(
    ShareaboutsFieldMixin, serializers.HyperlinkedIdentityField
):
    read_only = True

    def __init__(self, *args, **kwargs):
        view_name = kwargs.pop("view_name", None) or getattr(self, "view_name", None)
        super(ShareaboutsIdentityField, self).__init__(
            view_name=view_name, *args, **kwargs
        )
...
class PlaceTagIdentityField(serializers.HyperlinkedIdentityField):
    ...
    def __init__(self, *args, **kwargs):
        super(PlaceTagIdentityField, self).__init__(
            view_name="place-tag-detail", *args, **kwargs
        )
