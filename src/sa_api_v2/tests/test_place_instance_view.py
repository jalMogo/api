...
class TestPlaceInstanceView(APITestMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.invisible_path = reverse(
            "place-detail", kwargs=cls.invisible_request_kwargs
        )
    ...
    def test_GET_response_with_invalid_JWT_payload(self):
        ...
        """
        valid_jwt_public_incorrect_payload = jwt.encode(
            {"place_id": 99999999}, settings.JWT_SECRET, algorithm="HS256"
        ).decode()
    ...
    def test_GET_response_with_invalid_JWT_secret(self):
        ...
        """
        invalid_jwt_public = jwt.encode(
            {"place_id": self.place.id}, "invalid-secret-oh-no", algorithm="HS256"
        ).decode()
        request = self.factory.get(
    ...
    def test_PUT_response_as_group_submitter(self):
        ...
        GroupPermission.objects.create(
            group=group, submission_set="places", can_update=True
        )
    ...
    def test_PUT_response_as_owner_doesnt_change_submitter(self):
        ...
        self.assertEqual(
            data["properties"].get("submitter", {}).get("id"), self.submitter.id
        )
