...
class TestPlaceListView(APITestMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.submissions = [
            ...
            Submission.objects.create(
                place_model=cls.place, set_name="likes", dataset=cls.dataset, data="{}"
            ),
            Submission.objects.create(
                place_model=cls.place, set_name="likes", dataset=cls.dataset, data="{}"
            ),
        ...
        cls.ds_origin = Origin.objects.create(
            pattern="http://openplans.github.com", dataset=cls.dataset
        )

        cls.private_place = Place.objects.create(
            dataset=cls.dataset, geometry="POINT(7 8)", private=True,
        )
    ...
    def test_GET_response_for_multiple_specific_objects(self):
        ...
        for _ in range(10):
            places.append(
                Place.objects.create(
                    ...
                    data=json.dumps(
                        {"type": "ATM", "name": "K-Mart", "private-secrets": 42}
                    ),
                )
            )
        ...
        self.assertEqual(
            set([f["id"] for f in data["features"]]), set([p.pk for p in places[::2]])
        )
    ...
    def test_GET_text_search_response(self):
        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(0 0)",
            data=json.dumps({"foo": "bar", "name": 1}),
        ),
        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(1 0)",
            data=json.dumps({"foo": "bar", "name": 2}),
        ),
        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(2 0)",
            data=json.dumps({"foo": "baz", "name": 3}),
        ),
        Place.objects.create(
            dataset=self.dataset, geometry="POINT(3 0)", data=json.dumps({"name": 4})
        ),
        ...
        self.assertTrue(
            all(
                [
                    feature["properties"].get("foo") == "bar"
        ...
        self.assertTrue(
            all(
                [
                    feature["properties"].get("foo") in ("bar", "baz")
                    for feature in data["features"]
                ]
            )
        )
    ...
    def test_GET_unindexed_response(self):
        ...
        with patch.object(
            GeoSubmittedThingQuerySet, "filter_by_index"
        ) as patched_filter:
    ...
    def test_GET_nearby_response(self):
        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(0 0)",
            data=json.dumps({"new_place": "yes", "name": 1}),
        ),
        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(10 0)",
            data=json.dumps({"new_place": "yes", "name": 2,}),
        ),
        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(20 0)",
            data=json.dumps({"new_place": "yes", "name": 3,}),
        ),

        Place.objects.create(
            dataset=self.dataset,
            geometry="POINT(30 0)",
            data=json.dumps({"new_place": "yes", "name": 4,}),
        ),
    ...
    def test_POST_response(self):
        ...
        self.assertEqual(
            data["properties"]["jwt_public"],
            Place.objects.latest("id").make_jwt().decode(),
        )
    ...
    def test_GET_response_with_private_place(self):
        ...
        private_place = next(
            (x for x in data["features"] if x["id"] == self.private_place.id), None
        )
            ...
            [
                {
                    "properties": {
                        ...
                        "private-secrets": "The mayor loves this bench",
                        "id": place.id,
                        "url": "http://testserver/api/v2/aaron/datasets/ds/places/%s"
                        % (place.id,),
                    },
                    "type": "Feature",
                    "id": place.id,
    ...
    def test_model_update_clears_GET_cache_for_multiple_specific_objects(self):
        ...
        places[0].data = json.dumps(
            {"type": "ATM", "name": "K-Mart", "private-secrets": 43}
        )
        places[0].save()
        cache_buffer.flush()
