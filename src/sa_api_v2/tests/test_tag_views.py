...
from ..models import User, DataSet, Place, PlaceTag, Tag, Group, GroupPermission
from .test_views import APITestMixin
from ..views import PlaceTagInstanceView, PlaceTagListView
...
class TestPlaceTagInstanceView(APITestMixin, TestCase):
    def setUp(self):
        ...
        self.tags = [
            Tag.objects.create(name="status", dataset=self.dataset,),
        ]

        self.tags.extend(
            [
                Tag.objects.create(
                    name="approved", dataset=self.dataset, parent=self.tags[0]
                ),
                Tag.objects.create(
                    name="rejected", dataset=self.dataset, parent=self.tags[0]
                ),
            ]
        )
        ...
        self.unauthorized_user = User.objects.create_user(
            username="temp_user", password="lkjasdf"
        )
        self.authorized_user = User.objects.create_user(
            username="temp_user2", password="lkjasdf"
        )
        ...
        unauthorized_group = Group.objects.create(dataset=self.dataset, name="badgroup")
    ...
    def test_PUT_response(self):
        submission_data = json.dumps({"note": "Revised comment",})
...
class TestPlaceTagListView(APITestMixin, TestCase):
    def setUp(self):
        ...
        self.place_tags = [
            ...
            PlaceTag.objects.create(
                place=self.place, tag=self.tags[2], note="I reject this place!",
            ),
        ]
        ...
        self.other_tag = Tag.objects.create(
            name="other status", dataset=self.other_dataset,
        )

        other_group = Group.objects.create(
            dataset=self.other_dataset, name="othergroup"
        )
    ...
    def test_POST_response(self):
        tag_url = "http://testserver" + reverse(
            "tag-detail",
            args=[self.owner.username, self.dataset.slug, self.tags[1].id,],
        )
        place_url = "http://testserver" + reverse(
            "place-detail",
            args=[self.owner.username, self.dataset.slug, self.place.id,],
        )
        data = json.dumps(
            {"note": "this is a comment", "tag": tag_url, "place": place_url,}
        )
        ...
        request = self.factory.post(
            self.path, data=data, content_type="application/json"
        )
        response = self.view(request, **self.request_kwargs)
        self.assertStatusCode(response, 401)
    ...
    def test_POST_response_with_invalid_tag(self):
        tag_url = "http://testserver" + reverse(
            "tag-detail",
            args=[self.owner.username, self.other_dataset.slug, self.other_tag.id,],
        )
        place_url = "http://testserver" + reverse(
            "place-detail",
            args=[self.owner.username, self.dataset.slug, self.place.id,],
