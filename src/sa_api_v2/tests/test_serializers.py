...
class TestActionSerializer(TestCase):
    def setUp(self):
        ...
        comment = Submission.objects.create(
            dataset=dataset, place_model=place, set_name="comments"
        )
    ...
    def test_submission_action_attributes(self):
        serializer = ActionSerializer(
            self.comment_action, context={"request": RequestFactory().get("")},
        )
    ...
    def test_prejoined_place_action_attributes(self):
        ...
        serializer = ActionSerializer(
            action, context={"request": RequestFactory().get("")},
        )
...
class TestSubmissionSerializer(TestCase):
    ...
    def test_can_serlialize_a_null_instance(self):
        serializer = SubmissionSerializer(
            None, context={"request": RequestFactory().get("")},
        )
...
class TestDataSetSerializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.dataset = DataSet.objects.create(slug="data", owner_id=cls.owner.id)
        cls.place = Place.objects.create(dataset=cls.dataset, geometry="POINT(2 3)")
        Submission.objects.create(
            dataset=cls.dataset, place_model=cls.place, set_name="comments"
        )
        Submission.objects.create(
            dataset=cls.dataset, place_model=cls.place, set_name="comments"
        )
...
class TestFlavorSerializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.owner = User.objects.create(username="myuser")
        cls.dataset1 = DataSet.objects.create(slug="data", owner_id=cls.owner.id)
        cls.dataset2 = DataSet.objects.create(slug="data-2", owner_id=cls.owner.id)

        cls.flavor = Flavor.objects.create(display_name="myflavor",)

        cls.form1 = Form.objects.create(
            label="form1", dataset=cls.dataset1, flavor=cls.flavor,
        )

        cls.stages = [
            FormStage.objects.create(order=0, form=cls.form1,),
            FormStage.objects.create(order=1, form=cls.form1,),
        ]

        cls.html_module_content = "<p>Hey there!</p>"

        html_module = HtmlModule.objects.create(content=cls.html_module_content,)
        ...
        OrderedModule.objects.create(
            order=1, stage=cls.stages[0], radiofield=radio_field
        ),
        OrderedModule.objects.create(
            order=2, stage=cls.stages[0], htmlmodule=html_module,
        ),
        OrderedModule.objects.create(
            order=3, stage=cls.stages[0], groupmodule=related_group_module,
        )
        OrderedModule.objects.create(
            order=0, stage=cls.stages[1],
        )

        cls.form2 = Form.objects.create(
            label="form2", dataset=cls.dataset2, flavor=cls.flavor,
        )
    ...
    def test_attributes(self):
        serializer = FlavorSerializer(
            self.flavor, context={"request": RequestFactory().get("")},
        )
    ...
    def test_forms(self):
        ...
        form1 = next(
            form for form in serializer.data["forms"] if form["label"] == "form1"
        )
        self.assertTrue(form1.get("label"), self.form1.label)

        form2 = next(
            form for form in serializer.data["forms"] if form["label"] == "form2"
        )
...
class TestFlavorDeserializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        DataSet.objects.all().delete()
        cls.owner = User.objects.create(username="myuser")
        cls.dataset = DataSet.objects.create(slug="test-dataset", owner_id=cls.owner.id)
        cls.dataset2 = DataSet.objects.create(
            slug="kittitas-firewise-input", owner_id=cls.owner.id
        )
        cls.dataset3 = DataSet.objects.create(
            slug="bellevue-bike-share", owner_id=cls.owner.id
        )
        cls.dataset4 = DataSet.objects.create(
            slug="spokane-input", owner_id=cls.owner.id
        )
        cls.dataset5 = DataSet.objects.create(slug="pbdurham", owner_id=cls.owner.id)
        cls.dataset6 = DataSet.objects.create(
            slug="kittitas-vsp-input", owner_id=cls.owner.id
        )
        DataSet.objects.create(slug="snohomish-featured", owner_id=cls.owner.id)
        DataSet.objects.create(slug="snohomish-input", owner_id=cls.owner.id)
        DataSet.objects.create(slug="columbia-input", owner_id=cls.owner.id)
        DataSet.objects.create(slug="garfield-input", owner_id=cls.owner.id)
        DataSet.objects.create(slug="palouse-input", owner_id=cls.owner.id)
        DataSet.objects.create(slug="creatingcascadia-input", owner_id=cls.owner.id)
        DataSet.objects.create(slug="mississippi-input", owner_id=cls.owner.id)
        DataSet.objects.create(slug="mississippi-featured", owner_id=cls.owner.id)
        DataSet.objects.create(slug="mw", owner_id=cls.owner.id)
        DataSet.objects.create(slug="mw-featured-projects", owner_id=cls.owner.id)
        DataSet.objects.create(slug="mwfeatured", owner_id=cls.owner.id)
        pbdurham_projects = DataSet.objects.create(
            slug="pbdurham-projects", owner_id=cls.owner.id
        )
        Group.objects.create(
            dataset=cls.dataset5, name="administrators",
        )
        ...
        Group.objects.create(
            dataset=cls.dataset2, name="administrators",
        )
    ...
    def test_deserialize_flavor(self):
        ...
        if not form_serializer.is_valid():
            raise ValidationError(
                "FormSerializer failed with error{}:".format(form_serializer.errors)
            )
        ...
        flavor_serializer = FlavorFixtureSerializer(data=data["flavors"], many=True,)
        if not flavor_serializer.is_valid():
            raise ValidationError(
                "FlavorSerializer failed with error: {}".format(
                    flavor_serializer.errors
                )
            )
        ...
        self.assertEqual(
            [
                module.order
                for module in form.stages.get(order=1)
                .modules.get(order=5)
                .groupmodule.modules.get(order=1)
                .radiofield.options.first()
                .group_visibility_triggers.all()
            ],
            [2],
        )
        ...
        self.assertEqual(
            [
                stage.order
                for stage in form.stages.get(order=1)
                .modules.get(order=2)
                .radiofield.options.last()
                .stage_visibility_triggers.all()
            ],
            [2],
        )
        ...
        self.assertEqual(
            form.stages.first().visible_layer_groups.first().label, "layer1"
        )
        # assert that our MapViewport is valid:
        self.assertEqual(form.stages.all().get(order=2).map_viewport.zoom, 12)
        self.assertEqual(
            form.stages.all().get(order=2).map_viewport.transition_duration, None
        )
        ...
        self.assertEqual([option.label for option in options], ["White", "Black"])
        ...
        self.assertEqual(groupmodule.label, "a group module")
        self.assertEqual(
            groupmodule.modules.all().last().htmlmodule.label, "end of survey"
        )
    ...
    def test_deserialize_staging_flavors(self):
        ...
        if not layer_group_serializer.is_valid():
            raise AssertionError(
                "LayerGroupSerializer failed with error: {}".format(
        ...
        forms = next(
            flavor for flavor in flavors if flavor.slug == "my-water"
        ).forms.all()
        form = forms.get(label="mw-form")
        self.assertEqual(
            [
                trigger.order
                for trigger in form.stages.get(order=1)
                .modules.get(order=3)
                .groupmodule.modules.get(order=1)
                .radiofield.options.first()
                .group_visibility_triggers.all()
        ...
        self.assertEqual(
            [
                ...
                .modules.get(order=3)
                .groupmodule.modules.get(order=1)
                .radiofield.options.get(order=2)
                .group_visibility_triggers.all()
            ],
            [3],
        )
        self.assertEqual(
            [
                ...
                .modules.get(order=3)
                .groupmodule.modules.get(order=1)
                .radiofield.options.get(order=8)
                .group_visibility_triggers.all()
            ],
            [9],
        )
        ...
        forms = next(
            flavor for flavor in flavors if flavor.slug == "spokane-vsp"
        ).forms.all()
        form = forms.get(label="spokane-input-form")
        self.assertEqual(
            [
                stage.order
                for stage in form.stages.get(order=2)
                .modules.get(order=1)
                .checkboxfield.options.first()
                .stage_visibility_triggers.all()
            ],
            [3],
        )
        ...
        bellevue_form = next(
            flavor for flavor in flavors if flavor.slug == "bellevue-bike-share"
        ).forms.first()
        self.assertEqual(
