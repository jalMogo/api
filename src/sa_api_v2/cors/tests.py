...
class TestOriginClientAuth(TestCase):
    def setUp(self):
        ...
        self.permission1 = Origin.objects.create(
            pattern="github.com", dataset=self.dataset
        )
        self.permission2 = Origin.objects.create(
            pattern="localhost:*", dataset=self.dataset
        )
    ...
    def test_simple_origin_matching_on_not_first_origin(self):
        checker = OriginAuthentication()
        client_auth = checker.check_origin_permission(
            "http://localhost:8000", self.dataset
        )
