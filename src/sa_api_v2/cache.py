...
class CacheBuffer(object):
    ...
    def get_many(self, keys):
        ...
        all_results = dict(
            [(key, Undefined) for key in set(keys) - set(results.keys())]
        )
    ...
    def flush(self):
        ...
        if self.queue:
            ...
            for timeout, queue in list(timed_queues.items()):
...
class Cache(object):
    ...
    def get_keys_with_prefixes(self, *prefixes):
        ...
        for prefix in prefixes:
            ...
            keys.add(meta_key)
        logger.debug(
            'Keys with prefixes "%s": "%s"' % ('", "'.join(prefixes), '", "'.join(keys))
        )
        return keys
    ...
    def clear_instance(self, obj):
        ...
        other_keys = self.get_other_keys(**params) | set(
            [self.get_instance_params_key(obj.pk)]
        )
        # Clear all the keys
...
class DataSetCache(Cache):
    ...
    def get_instance_key(self, **params):
        return ":".join(
            ["dataset-instance", params["owner_username"], params["dataset_slug"]]
        )
    ...
    def get_permissions_key(self, **params):
        return ":".join(
            ["dataset-permissions", params["owner_username"], params["dataset_slug"]]
        )
    ...
    def get_other_keys(self, **params):
        return set(
            [self.get_instance_key(**params), self.get_permissions_key(**params)]
        )
...
class PlaceCache(Cache):
    ...
    def get_instance_params(self, place_obj):
        ...
        params.update(
            {"place_id": place_obj.pk, "thing_id": place_obj.pk, "thing_type": "place"}
        )
        return params

    def get_request_prefixes(self, **params):
        owner, dataset, place = list(
            map(params.get, ("owner_username", "dataset_slug", "place_id"))
        )
...
class SubmissionCache(Cache):
    ...
    def get_instance_params(self, submission_obj):
        params = self.place_cache.get_cached_instance_params(
            submission_obj.place_model_id, lambda: submission_obj.place_model
        ).copy()
        params.update(
            {
    ...
    def get_other_keys(self, **params):
        dataset_id, place_model_id, submission_set_name = list(
            map(params.get, ["dataset_id", "place_id", "submission_set_name"])
        )
        dataset_serialized_data_keys = self.dataset_cache.get_serialized_data_keys(
            dataset_id
        )
        place_serialized_data_keys = self.place_cache.get_serialized_data_keys(
            place_model_id
        )
        return dataset_serialized_data_keys | place_serialized_data_keys

    def get_request_prefixes(self, **params):
        owner, dataset, place_model, submission_set_name, submission = list(
            map(
                params.get,
                [
                    "owner_username",
        ...
        # would that work?
        specific_instance_path = reverse(
            "submission-detail",
            args=[owner, dataset, place_model, submission_set_name, submission],
        )
        general_instance_path = reverse(
            "submission-detail",
            args=[owner, dataset, place_model, "submissions", submission],
        )
        specific_collection_path = reverse(
            "submission-list", args=[owner, dataset, place_model, submission_set_name]
        )
        general_collection_path = reverse(
            "submission-list", args=[owner, dataset, place_model, "submissions"]
        )
        specific_all_path = reverse(
            "dataset-submission-list", args=[owner, dataset, submission_set_name]
        )
        general_all_path = reverse(
            "dataset-submission-list", args=[owner, dataset, "submissions"]
        )
        place_instance_path = reverse(
            "place-detail", args=[owner, dataset, place_model]
        )
        ...
        prefixes.update(
            [
                specific_instance_path,
                general_instance_path,
                specific_collection_path,
                general_collection_path,
                specific_all_path,
                general_all_path,
                place_instance_path,
                place_collection_path,
                dataset_instance_path,
                dataset_collection_path,
                action_collection_path,
            ]
...
class PlaceTagCache(Cache):
    ...
    def get_instance_params(self, place_tag_obj):
        params = self.place_cache.get_cached_instance_params(
            place_tag_obj.place_id, lambda: place_tag_obj.place
        ).copy()
        params.update(
...
class AttachmentCache(Cache):
    ...
    def get_instance_params(self, attachment_obj):
        ...
        params.update(
            {"attachment": attachment_obj.name, "attachment_id": attachment_obj.pk,}
        )
        return params
    ...
    def get_submission_attachment_request_prefixes(self, **params):
        owner, dataset, place, submission_set_name, submission = list(
            map(
                params.get,
                [
                    "owner_username",
        ...
        specific_instance_path = reverse(
            "submission-detail",
            args=[owner, dataset, place, submission_set_name, submission],
        )
        general_instance_path = reverse(
            "submission-detail", args=[owner, dataset, place, "submissions", submission]
        )
        specific_collection_path = reverse(
            "submission-list", args=[owner, dataset, place, submission_set_name]
        )
        general_collection_path = reverse(
            "submission-list", args=[owner, dataset, place, "submissions"]
        )
        specific_all_path = reverse(
            "dataset-submission-list", args=[owner, dataset, submission_set_name]
        )
    ...
    def get_other_keys(self, **params):
        ...
        if params["thing_type"] == "submission":
            thing_serialized_data_keys = self.submission_cache.get_serialized_data_keys(
                thing_id
            )
        else:
            thing_serialized_data_keys = self.place_cache.get_serialized_data_keys(
                thing_id
            )
