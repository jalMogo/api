...
class DataSetFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        qs = model_admin.get_queryset(request)
        qs = (
            qs.order_by("dataset__slug")
            .distinct("dataset__slug")
            .values("dataset__slug")
        )
...
class SubmittedThingAdmin(admin.OSMGeoAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        FormWithJSONCleaning = super(SubmittedThingAdmin, self).get_form(
        ...
        FormWithJSONCleaning.base_fields["data"].widget = PrettyAceWidget(
            mode="json", width="100%", wordwrap=True, theme="jsoneditor"
        )
...
class InlineOriginAdmin(admin.StackedInline):
    ...
    def edit_url(self, instance):
        if instance.pk is None:
            ...
            return '<a href="%s"><strong>Edit permissions</strong></a>' % (
...
class DataSetAdmin(DjangoObjectActions, admin.ModelAdmin):
    ...
    def clone_dataset(self, request, obj):
        ...
        try:
            ...
            new_obj_edit_url = reverse(
                "admin:sa_api_v2_dataset_change", args=[new_obj.pk]
            )
            messages.success(request, "Cloning dataset. Please give it a few moments.")
            return HttpResponseRedirect(new_obj_edit_url)
        except Exception as e:
            messages.error(
                request, "Failed to clone dataset: %s (%s)" % (e, type(e).__name__)
            )
...
class PlaceAdmin(SubmittedThingAdmin):
    model = models.Place
    inlines = [InlinePlaceTagAdmin, InlineAttachmentAdmin]
    readonly_fields = ("jwt_public_token", "api_path")

    def jwt_public_token(self, instance):
        return instance.make_jwt().decode()

    def api_path(self, instance):
        path = reverse(
            "place-detail",
            args=[instance.dataset.owner, instance.dataset.slug, instance.id],
        )
        return '<a href="{0}">{0}</a>'.format(path)
...
class SubmissionAdmin(SubmittedThingAdmin):
    ...
    list_display = (
        SubmittedThingAdmin.list_display[:-1]
        + ("place_model", "set_",)
        + SubmittedThingAdmin.list_display[-1:]
    )  # keep the 'data' column at the end
    ...
    def api_path(self, instance):
        path = reverse(
            "submission-detail",
            args=[
                instance.dataset.owner,
                instance.dataset.slug,
                instance.place_model.id,
                instance.set_name,
                instance.id,
            ],
        )
...
class FormFieldOptionInlineForm(ModelForm):
    group_visibility_triggers = ModelMultipleChoiceField(
        ...
        help_text=str(
            models.FormFieldOption._meta.get_field(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(self.instance, "field") and hasattr(
            self.instance.field, "nestedorderedmodule"
        ):
            ...
            self.fields[
                "group_visibility_triggers"
            ].queryset = models.NestedOrderedModule.objects.select_related(
                "group"
            ).filter(
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        # OrderedModule:
            ...
            raise FieldDoesNotExist(
                "db_field name does not exist: {}".format(db_field.name)
            )
        ...
        return super(AbstractFormModuleAdmin, self).formfield_for_foreignkey(
            db_field, request, **kwargs
        )
...
class NestedOrderedModuleAdmin(AbstractFormModuleAdmin):
    ...
    def _get_related_queryset(self, queryset, field_name):
        ...
        return queryset.prefetch_related("nestedorderedmodule").filter(
            Q(orderedmodule=None)
            & (
                Q(nestedorderedmodule__id=self.nestedorderedmodule_id)
                | Q(nestedorderedmodule=None)
            ),
        )
    ...
    def edit_url(self, instance):
        if instance.pk is None:
            ...
            return format_html(
                ...
                reverse(
                    "admin:sa_api_v2_nestedorderedmodule_change", args=[instance.pk]
                ),
            )
...
class FormStageInline(SortableInlineAdminMixin, admin.StackedInline):
    ...
    def summary(self, instance):
        ...
        if related_module is None:
            ...
            return "module id: {}, {}".format(instance.id, related_module,)
